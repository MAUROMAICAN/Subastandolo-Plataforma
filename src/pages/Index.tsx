        ...
        <div className="sticky top-12 sm:top-14 z-30 bg-background/80 backdrop-blur-xl border-b border-border/30">
          <div className="container mx-auto px-3 sm:px-4">
            <div className="flex justify-center py-2.5">
              <div className="inline-flex items-center gap-1 p-1 rounded-xl bg-secondary/60 dark:bg-white/5 border border-border/40 dark:border-white/10 overflow-x-auto scrollbar-hide max-w-full">
                {filterButtons.map(f => (
                  <button
                    key={f.key}
                    onClick={() => setActiveFilter(f.key)}
                    className={`relative flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-[11px] sm:text-xs font-semibold whitespace-nowrap transition-all duration-200 ${activeFilter === f.key
                      ? "bg-primary text-primary-foreground shadow-sm"
                      : "text-muted-foreground hover:text-foreground hover:bg-secondary/80 dark:hover:bg-white/5"
                      }`}
                  >
                    <f.icon className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${activeFilter === f.key ? "" : "opacity-60"}`} />
                    <span className="hidden sm:inline">{f.label}</span>
                    <span className="sm:hidden">{f.label}</span>
                    <span className={`text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center leading-none ${activeFilter === f.key
                      ? "bg-primary-foreground/20 text-primary-foreground"
                      : "bg-muted/80 dark:bg-white/10 text-muted-foreground"
                      }`}>{f.count}</span>
                  </button>
                ))}
                {user && (
                  <button
                    ...
                    className={`relative flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-[11px] sm:text-xs font-semibold whitespace-nowrap transition-all duration-200 ${activeFilter === "my_bids"
                      ? "bg-primary text-primary-foreground shadow-sm"
                      : "text-muted-foreground hover:text-foreground hover:bg-secondary/80 dark:hover:bg-white/5"
                      }`}
                  >
                    <Gavel className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${activeFilter === "my_bids" ? "" : "opacity-60"}`} />
                    <span>Mis Pujas</span>
                    <span className={`text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center leading-none ${activeFilter === "my_bids"
                      ? "bg-primary-foreground/20 text-primary-foreground"
                      : "bg-muted/80 dark:bg-white/10 text-muted-foreground"
