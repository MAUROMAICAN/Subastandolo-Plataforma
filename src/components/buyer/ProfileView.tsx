...
import { Loader2, ArrowLeft, User, MapPin, CreditCard } from "lucide-react";
...
export default function ProfileView({ profile, user, refreshProfile, onBack }: ProfileViewProps) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            // Update local state so progress bar reflects changes immediately
            setCedulaPhotoUrl(finalCedulaUrl);
            // Refresh profile context in background (does not reset this component's state)
            refreshProfile();
        }
        setUpdating(false);
    };
